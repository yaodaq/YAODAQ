...
yaodaq::ProcessTransport::~ProcessTransport()
{
  close();
}
...
bool yaodaq::ProcessTransport::open()
{
  ...
  for(auto& variable : env)
  {
    ...
    if(pos != std::string::npos)
    {
      ...
      env_storage.erase(std::remove_if(env_storage.begin(),env_storage.end(),
...
while (true)
{
    ...
    auto readFd = [&](const int fd,const short revents,const std::string_view channel)
    {
        ...
        while(true)
        {
            ...
            {
                ret.emplace_back(
                    ByteBufferView(buffer, static_cast<size_t>(n)),
                    channel
                );
...
bool yaodaq::ProcessTransport::update()
{
  if(!m_running) return true;
  int status{0};
  auto result = ::waitpid(m_pid,&status,WNOHANG);
...
bool yaodaq::ProcessTransport::terminate()
{
  return sendSignal(SIGTERM);
}

bool yaodaq::ProcessTransport::kill()
{
