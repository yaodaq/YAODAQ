...
public:
    ...
    std::uint16_t getFineTimeTick() const noexcept
    {
        return fine_time;
    }
    double getFineTime() const noexcept
    {
        return fine_time*m_tick_time;
    }
    std::uint8_t getSide() const noexcept
    {
        return eta;
    }
    std::uint32_t getOutBCID() const noexcept
    {
        return out_bcid;
    }
    std::uint32_t getBCID() const noexcept
    {
        return bcid;
    }
private:
...
public:
    ...
    void push_back(const DecodedRawData& decoded) // decouple the hits if eta1 and eta2 are save in the same DecodedRawData
    {
      ...
      if(decoded.is_trigger())
      {
        ...
        else trigger_hits.push_back(Hit(decoded));
      }
      else
      {
        if(decoded.has_both_side())
        ...
        else hits.push_back(Hit(decoded));
      }
    }
    void clear()
    {
