...
std::vector<std::byte> yaodaq::YAODAQJSONCodec::encode( const yaodaq::Message& msg ) const
{
  nlohmann::json json;
  json["meta"]                               = nlohmann::json::object();
  json["meta"]["time"]                       = msg.time();
  ...
  switch( msg.type() )
  {
    ...
    case Message::Type::RawData:
    {
      ...
      json["payload"]       = raw.payload();
      json["meta"]["topic"] = raw.topic();
...
std::unique_ptr<yaodaq::Message> yaodaq::YAODAQJSONCodec::decode( const TransportPacket& data ) const
{
  ...
  switch( type )
  {
    ...
    case Message::Type::RawData:
    {
      ...
      meta["topic"].get( topic );
      simdjson::dom::array arr;
      json["payload"].get( arr );
      std::string str;
